library(pacman)
p_load(dplyr, terra, sf, geosphere, tidyterra)

args <- commandArgs(trailingOnly = TRUE)
wdpaid <- args[1]

fn <- paste0('wdpa_wdpaid_', wdpaid, '.gpkg') # Input geopackage file
lname <- 'landcover' # Output layer name
v <- vect(fn, layer = "sql_statement")
# Check if CRS is valid and reproject to WGS84 if needed
if (is.na(crs(v)) || crs(v) == "") {
    stop("Vector has no CRS defined")
}
if (crs(v, describe = TRUE)$code != "4326") {
    v <- project(v, "epsg:4326") # Ensure geometries are in WGS84
}
v <- filter(v, wdpaid == wdpaid) # Filter to specific WDPA ID
v <- union(v) # Dissolve to single part if multipart
v$fixed <- ""

is_valid <- is.valid(v)
if (!is_valid) {
    v <- makeValid(v)
    v$fixed <- "makeValid"
}

is_valid <- is.valid(v)
if (!is_valid) {
    v <- buffer(v, width = 0)
    v$fixed <- "buffer_0"
}

is_valid <- is.valid(v)
if (!is_valid) {
    v <- union(v)
    v$fixed <- "union"
}

is_valid <- is.valid(v)
if (!is_valid) {
    v$fixed <- "invalid"
}

# Extract landcover data
files <- list.files("/home/ortega/Rasters/COP_landcover/", pattern = ".tif$", full.names = TRUE)
landcover_summary <- lapply(files, function(file) {
landcover_raster <- terra::rast(file)
landcover_data <- terra::extract(landcover_raster, v, ID = FALSE)
res <- table(landcover_data[[1]]) %>% as.data.frame() %>%
pivot_wider(names_from = Var1, values_from = Freq, values_fill = 0)
return(res)
}) %>% bind_rows() %>%
summarise(across(everything(), mean))

v <- cbind(v, landcover_summary)
v$wdpaid <- wdpaid

v <- st_as_sf(v)
v$wdpaid <- wdpaid
sf::st_write(v, dsn = fn, layer = lname, delete_layer = TRUE)